 ...
 *
 * Ported from Docker: https://github.com/docker/docker/blob/master/pkg/namesgenerator/names-generator.go
...
object NameGenerator {
	...
	def apply(rand: Random): String = {
		...
		val ret = adj + "_" + name
		if(ret.equals("boring_wozniak"))// Steve Wozniak is not boring
			apply(rand)
		else ret
	}
	...
	val surnames = List(
		...
		"hoover",		// Erna Schneider Hoover revolutionized modern communication by inventing a computerized telephon switching method. https://en.wikipedia.org/wiki/Erna_Schneider_Hoover
